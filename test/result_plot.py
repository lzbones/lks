...
# ---------- 设置输出目录为 test/ ----------
matplotlib_use = "Agg"   # 非交互后端，适合后台运行

try:
    import matplotlib
    matplotlib.use(matplotlib_use)

    plt.rcParams["font.family"] = ["PingFang TC", "Songti SC", "Arial Unicode MS", "DejaVu Sans"]
except ImportError:
    pass
...
# ---------- 读取数据 ----------
lane_file = os.path.join(script_dir, "trajectory.csv")
traj_file = os.path.join(script_dir, "trajectory.csv")

# 从项目根目录查找 CSV 文件
project_root = os.path.dirname(script_dir)
lane_csv = os.path.join(project_root, "test", "lane.csv")
traj_csv = os.path.join(project_root, "test", "trajectory.csv")

print(f"读取: {traj_csv}")
print(f"读取: {lane_csv}")

lane = np.genfromtxt(lane_csv, delimiter=",")
traj = np.genfromtxt(traj_csv, delimiter=",", skip_header=1)
